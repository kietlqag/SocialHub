...
import { type DashboardTable, type DashboardWidget } from "../services/dashboards";
...
const buildSampleValue = (field: Field, index: number) => {
  if (field.sampleData) {
    return field.sampleData.replace(/\{\{\s*index\s*\}\}/gi, String(index + 1));
  }
  ...
  if (type.includes("currency") || type.includes("number")) {
    return Intl.NumberFormat("en-US", { style: type.includes("currency") ? "currency" : "decimal", currency: "USD" }).format(
      1000 + index * 42,
    );
  }
...
const generateSampleRecords = (fields: Field[], prefix?: string): SampleRecord[] => {
  if (!fields.length) return [];
  return Array.from({ length: SAMPLE_RECORD_COUNT }).map((_, idx) => ({
...
const timeRangeOptions = ["7d", "30d", "12m"] as const;
...
export function CreatedDashboardView({ isOpen, onClose, dashboard, onSave }: CreatedDashboardViewProps) {
  ...
  const lineData = useMemo(() => {
    const labels = getTimelineLabels(timeRange);
    const base = Math.max(40, totalFields * 10 + totalTables * 24);
    const multiplier = timeRange === "7d" ? 1 : timeRange === "30d" ? 1.25 : 1.8;
    return labels.map((label, index) => ({
      label,
      value: Math.round(base * multiplier * (index + 1) / labels.length),
    }));
  }, [timeRange, totalFields, totalTables]);
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <DialogTitle className="text-2xl text-gray-900 mt-1">{dashboard.name || "AI dashboard"}</DialogTitle>
                    <DialogDescription className="text-sm text-gray-600 mt-1">
                      {dashboard.description || `Your AI-generated dashboard with ${totalTables} tables and ${totalFields} fields.`}
                    </DialogDescription>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Button variant="outline" onClick={handleCopyCode} disabled={!dashboard.componentCode}>
                  {copied ? "Code copied" : "Copy layout code"}
                </Button>
                <Button onClick={handleSave} disabled={isSaving} className="gap-2">
                  {isSaving ? "Saving..." : <><Save className="w-4 h-4" /> Save to MongoDB</>}
                </Button>
              </div>
            </div>
            ...
            <section className="space-y-4">
              ...
              {hasTables ? (
                ...
                <Card className="p-10 text-center text-gray-500 border border-dashed border-gray-200">
                  No tables were generated for this dashboard.
                </Card>
              )}
            </section>
      ...
      {data.map((point) => (
        <div key={point.label} className="flex flex-col items-center gap-2">
          <div className="flex h-full w-6 items-end">
            <span
              className="block w-full rounded-full bg-gradient-to-t from-slate-900 to-slate-500"
...
function TableWidget({ table }: TableWidgetProps) {
  ...
  const sampleRecords = useMemo(() => generateSampleRecords(table.fields, table.id), [table.fields, table.id]);
  const filteredRecords = useMemo(() => {
    if (!searchTerm.trim()) return sampleRecords;
    const needle = searchTerm.toLowerCase();
    return sampleRecords.filter((record) =>
      Object.values(record.values).some((value) => value.toLowerCase().includes(needle)),
    );
  }, [sampleRecords, searchTerm]);
